...
/* Disposition en cercle quand il n'y a pas de coordonnees */
...
/* Disposition a partir des coordonnees reelles */
...
void visualize(const TSPInstance *inst) {
    ...
    if (!has_coords)
        std::printf("[Visualizer] Pas de coordonnees : disposition en cercle.\n");
    ...
    while (window.isOpen()) {
        ...
        /* Dessin des noeuds uniquement, sans aretes */
        for (int i = 0; i < n; i++) {
